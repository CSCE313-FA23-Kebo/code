...
sigprocmask(SIG_BLOCK, &new_set, NULL); // Union
...
sigaddset(&new_set, SIGINT);
sigaddset(&new_set, SIGTERM);
sigprocmask(SIG_UNBLOCK, &new_set, NULL); // current (Intersection) !{SIGINT, SIGTERM}
...
sigprocmask(SIG_SETMASK, &new_set, NULL); // Overwrite the current signals
